...
static void *coalesce(void *bp)
{
	...
	if (preva && nexta)//there is no need to coalesce都不是空闲块
		return bp;
	else if (nexta && (!preva)) //the previous block is 0下一块空闲
	{
	...
	else if (preva && (!nexta)) //the next block is 0上一块空闲
	{
	...
	else //both previous and next blocks are 0上一块和下一块都为空闲块
	{
