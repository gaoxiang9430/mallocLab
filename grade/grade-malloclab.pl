...
#
@name=split(/\//,$infile); 
@output = "res/@name[1]_res.txt";

print "Part 1: Compiling the driver\n\n";
if (system("(cd $tmpdir; make mdriver)") != 0){
	open(out, ">res/@name[1]_res.txt") or die $!;
...
if(system("(.\/$tmpdir/mdriver -t $TRACEDIR -vg > @output)") != 0 )
{
	open(out, ">res/@name[1]_res.txt") or die $!;
	print out ("runtime error\n");
	exit;
}

#"res/"+name+"_mm.c_res.txt"
