...
#define HEAD(bp) ((void *)(bp)-WSIZE)//得到头部的指针
#define LEFT(bp) ((void *)(bp))//左儿子
#define RIGHT(bp) ((void *)(bp)+WSIZE)//右儿子，这里用二叉搜索树维护
#define PRNT(bp) ((void *)(bp)+DSIZE)//父亲节点
#define BROS(bp) ((void *)(bp)+(3*WSIZE))//兄弟节点（这里特指块大小相同的节点）
#define FOOT(bp) ((void *)(bp)+SIZE(HEAD(bp))-DSIZE)//得到脚部的指针
...
#define PUT_LEFT_CHILD(bp,val) (PUT(LEFT(bp),(int)val))//init left child
#define PUT_RIGHT_CHILD(bp,val) (PUT(RIGHT(bp),(int)val))//init right child
#define GET_LEFT_CHILD(bp) (GET(LEFT(bp)))//get left child
#define GET_RIGHT_CHILD(bp) (GET(RIGHT(bp)))//get right child
...
#define GET_HEAD(bp) (GET(HEAD(bp)))//get the point of head
#define GET_FOOT(bp) (GET(FOOT(bp)))//get the point of foot
...
#define GET_PAR(bp) (GET(PRNT(bp)))//get the point of parent
#define GET_BRO(bp) (GET(BROS(bp)))//get the point of brother
int mm_init ();//init the heap
void *mm_malloc (size_t size);//apply for size place from the heap
void mm_free (void *bp);//free the bp point which stand for the block
void *mm_realloc (void *bp,size_t size);//renew malloc the size for bp point
...
static void *heap_list_ptr = 0//初始时刻的堆指针，一般指向堆得开始处
static void *my_tree = 0;//二叉搜索树的根节点
...
void *mm_malloc(size_t size)
{
	...
	if (size <= 0)
		return 0;
	my_size = size + 8;//加8是因为头部和尾部总共8个字节
	if (my_size <= 24)
		my_size = 24;  //最小24个字节
	else
		my_size = ALIGNED(my_size);//对齐处理
